...
@PIPELINES.register_module()
class RandomFlip3D(RandomFlipV2):
...
class RandomFlip3DV2:
    def __call__(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if len(data['bbox3d_fields']):  # training
            ...
            if 'transformation_3d_flow' not in data:
                data['transformation_3d_flow'] = []
            rotation = np.eye(3)
            if data['pcd_horizontal_flip']:
                rotation = np.array([[1, 0, 0], [0, -1, 0], [0, 0, 1]]) @ rotation
                if "points" in data:
                    data["points"].flip("horizontal")
                data['transformation_3d_flow'].extend(['HF'])
            if data['pcd_vertical_flip']:
                rotation = np.array([[-1, 0, 0], [0, 1, 0], [0, 0, 1]]) @ rotation
                if "points" in data:
                    data["points"].flip("vertical")
                data['transformation_3d_flow'].extend(['VF'])
...
class GlobalRotScaleTransV2:
    ...
    def __call__(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        elif isinstance(self.rot_lim, float): # tta
            if "points" in data:
                data["points"].rotate(-self.rot_lim)

            angle = torch.tensor(self.rot_lim)
            rot_sin = torch.sin(angle)
            rot_cos = torch.cos(angle)
            rot_mat_T = torch.tensor([[rot_cos, -rot_sin, 0],
                                                [rot_sin, rot_cos, 0],
                                                [0, 0, 1]])
            rotation = np.eye(3) @ rot_mat_T.numpy()
